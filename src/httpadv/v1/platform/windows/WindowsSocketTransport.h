...
public:
  explicit WindowsClient(SocketHandle socketHandle)
...
public:
  explicit WindowsServer(std::uint16_t port)
  ...
  std::unique_ptr<IClient> accept() override {
    ...
    return std::make_unique<WindowsClient>(accepted);
  }
...
class WindowsPeer : public IPeer {
public:
